...
http
  .createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/') {
      ...
      // 复制 Dockerfile 到项目目录,内置于项目中,clone时默认拉取master分支
      fs.copyFileSync(
        path.resolve(`./Dockerfile`),
        path.resolve(projectDir, './Dockerfile'),
      );

      // 复制 .dockerignore 到项目目录,内置于项目中
      fs.copyFileSync(
      ...
      // 创建 docker 镜像
      execSync(
        `docker build . --network=host -t  ${data.repository.name}-image:latest`,
        {
          stdio: 'inherit',
          cwd: projectDir,
        },
      ...
      // 创建 docker 容器
      execSync(
        `docker run --network=host -d -p 8888:80 --name ${data.repository.name}-container  ${data.repository.name}-image:latest`,
        {
          stdio: 'inherit',
        },
      );
