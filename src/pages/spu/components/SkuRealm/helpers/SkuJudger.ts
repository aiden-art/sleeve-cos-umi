...
class SkuJudger {
  ...
  getCompleteSkuCode() {
    ...
    let skuCode;
    if (this.skuPending) {
      skuCode = this.skuPending.getSkuCode();
    }
  ...
  isExistInAllSkuCode(path: string) {
  ...
  _getCellCode(spec: SkuSpecModel) {
    return spec.keyId + '-' + spec.valueId;
  }
